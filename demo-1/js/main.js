...
$(function () {
    ...
    var middleAxisLabel = g.append("g")
        .attr("class", "x axis")
        .attr("transform", "translate(0," + (height / 2) + ")").call(middleAxis)
    ...
    // Add labels
    var topLabel = svg.append('text').text('Data Space (domain)').attr('x', padding.left + width / 2).attr('y', 15).attr('class', 'axis-label');

    var bottomLabel = svg.append('text').text('Visual Space (range)').attr('x', padding.left + width / 2).attr('y', height + padding.top + padding.bottom - 5).attr('class', 'axis-label');
    // Define color scale
    ...
    var draw = function (data) {
        ...
        paths.enter()
            ...
            .style("stroke", function (d, i) {
                return colors[i] == undefined ? 'gray' : colors[i];
            });
