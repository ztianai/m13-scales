...
$(function() {
    ...
    var topScale = d3.scalePoint().domain(domain).rangeRound(topRange, step);

    // Declare a scale the shows the range (you won't typically do this)
    var middleScale = d3.scalePoint().domain(domain).rangeRound(middleRange, step);

    // Declare a scale showing the "function" in between domain and range
    var bottomScale = d3.scalePoint().domain(domain).rangeRound(bottomRange, step);
